...
const KpiCard = ({ title, value, change, icon, iconBg, isUrgent = false }) => {
    ...
    return (
        <div className="bg-white p-5 rounded-xl shadow-md flex items-center gap-5 transition-transform transform hover:-translate-y-1 hover:shadow-lg">
            <div className={`flex-shrink-0 w-14 h-14 rounded-full flex items-center justify-center text-white ${iconBg}`}>
                {icon}
            </div>
...
function App() {
    ...
    const { filteredData, previousPeriodData } = useMemo(() => {
        if (!dateRange.start || !dateRange.end) return { filteredData: [], previousPeriodData: [] };
        ...
        const currentFiltered = allData.filter(d => {
            const dateMatch = d.order_date >= start && d.order_date <= end;
        ...
        return { filteredData: currentFiltered, previousPeriodData: prevFiltered };
    ...
    const chartAndTableData = useMemo(() => {
        ...
        const topCustomerData = Object.values(data.reduce((acc, d) => {
    ...
    // ==================================================================
    // === NEW - This function creates the label for the doughnut charts ===
    // ==================================================================
    ...
    const renderCustomizedLabel = ({ cx, cy, midAngle, innerRadius, outerRadius, value, percent }) => {
        ...
        // Don't render a label if the segment is too small
        if (percent < 0.05) {
            return null;
        }
        ...
        return (
            ...
            >
                {/* Line 1: The value (e.g., 150) */}
                <tspan x={x} dy="-0.5em">{value}</tspan>
                {/* Line 2: The percentage (e.g., 75%) */}
    ...
    if (loading) {
        return <div className="flex items-center justify-center min-h-screen bg-gray-100 text-gray-600">Loading dashboard...</div>;
    }
    if (error) {
        return <div className="flex items-center justify-center min-h-screen bg-red-100 text-red-700 p-4">Error: {error}</div>;
    }
                ...
                <main>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            ...
                            <ResponsiveContainer>
                                <PieChart>
                                    <Pie 
                                        ...
                                        cx="50%" 
                                        cy="50%" 
                                        innerRadius={50} // This makes it a doughnut chart
                                        outerRadius={80} 
                                        paddingAngle={3}
                                        labelLine={false}
                        ...
                        <ChartContainer title="Top 5 Customers by Service Type">
                            <ResponsiveContainer>
                                <BarChart data={chartAndTableData.topCustomerChart} layout="vertical" margin={{ top: 5, right: 20, left: 100, bottom: 5 }}>
                                    <CartesianGrid strokeDasharray="3 3" />
                                    <XAxis type="number" />
                                    <YAxis type="category" dataKey="customer" width={100} interval={0} />
                                    <Tooltip />
                                    <Legend />
                        ...
                        <ChartContainer title="Orders by Department">
                            <ResponsiveContainer>
                                <BarChart data={chartAndTableData.ordersByDeptChart} layout="vertical" margin={{ top: 5, right: 20, left: 100, bottom: 5 }}>
                                    <CartesianGrid strokeDasharray="3 3" />
                                    <XAxis type="number" />
                                    <YAxis type="category" dataKey="dept" width={100} interval={0}/>
                                    <Tooltip />
                                    <Legend />
                        ...
                        <ChartContainer title="Urgent Count by Department">
                            <ResponsiveContainer>
                                <BarChart data={chartAndTableData.urgentByDeptChart} layout="vertical" margin={{ top: 5, right: 20, left: 100, bottom: 5 }}>
                                    <CartesianGrid strokeDasharray="3 3" />
